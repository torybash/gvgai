...
public class Agent extends AbstractPlayer{
	...
	public ACTIONS act(StateObservation so, ElapsedCpuTimer et) {
        ...
        Node currentNode = new Node(so, new LinkedList<Integer>(), so.getAvatarPosition());
        q.add(currentNode);
...
//        	System.out.println("Node initialized. Depth: " + depth + " action list: " + getActionList(n.list) + ", score: " + score + ", won: " + won);
//        	System.out.println("last pos: " + lastPos + ", current pos: " +);
        	...
        	if (expandFromNode){
        		if (depth == 0){
	        		...
	        		for (int i = 0; i < actions.length; i++) {
						...
						Node newN = new Node(n.state.copy(), (LinkedList<Integer>)n.list.clone(), lastPos);
						newN.list.add(act);
						q.add(newN);
					}
        		}else{
	        		...
	        		for (int i = 0; i < actions.length; i++) {
	        			...
	        			Node newN = new Node(n.state, (LinkedList<Integer>)n.list.clone(), n.state.getAvatarPosition());
	        			newN.list.add(act);
	        			q.add(newN);
	        			break;
	        		}
        ...
        for (int i = 0; i < bestActions.length; i++) {
			...
			lastScore = score;
//			System.out.println("bestActions["+i+"("+actions[i]+")]: " + bestActions[i]);
//			System.out.println("is wall action? " + wallActions[i]);
		}
        
        if (allScoresSame || highestScore < -999999) bestAction = r.nextInt(actions.length);
		
//        System.out.println("Returning.. Iterations: " + numIters + " action: "+ actions[bestAction] + (allScoresSame?" -- allScoresSame!":"") + ", highestScore: "+ highestScore);
